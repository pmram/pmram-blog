...
# CONNECT TO DB
if os.environ.get('DATABASE_URL?sslmode=require').replace('postgres://', 'postgresql://'):
...
def register():
    ...
    if request.method == 'POST':
        if register_form.validate_on_submit():
            if db.session.query(User).where(User.email == request.form.get('email')).first():
...
def login():
    ...
    if request.method == 'POST':
        if login_form.validate_on_submit():
            user = db.session.query(User).where(User.email == request.form.get('email')).first()
            if user:
                ...
                flash('No user found for the indicated email, try again!')
